...
INPUT_ROOT="${INPUT_ROOT:-${ROOT}/data/render_repair_imgs}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT}/outputs/render_repair_imgs}"
...
input_image_count=$(find "${INPUT_ROOT}" -mindepth 2 -maxdepth 2 -type f \
    \( -iname '*.png' -o -iname '*.jpg' -o -iname '*.jpeg' -o -iname '*.webp' \) \
    -print | wc -l | tr -d '[:space:]')
expected_task_count=$((input_image_count * ${#motion_ids[@]}))
echo "[plan] images=${input_image_count} motions=${#motion_ids[@]} total_tasks=${expected_task_count}"
if [[ "${expected_task_count}" -ne 32 ]]; then
    echo "[warning] expected 32 tasks for the current request, but discovered ${expected_task_count}" >&2
fi
