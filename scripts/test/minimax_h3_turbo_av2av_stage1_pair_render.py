#!/usr/bin/env python3
"""Render MiniMax-H3 Turbo AV-to-AV stage-1 classification pairs.
...
DEFAULT_LORA_PATH = DEFAULT_DIFFSYNTH_ROOT / (
    "lora_models/larryvrh/MiniMax-H3-Turbo-Lora/"
    "minimax_h3_turbo_v4_step600_ema.safetensors"
)
DEFAULT_OUTPUT_DIR = Path("results/minimax_h3_turbo_av2av_stage1_pairs")
H3_FPS = 24
...
def trim_gt_video(src: Path, dst: Path, frame_count: int, fps: int, width: int, height: int) -> None:
    ...
    cmd = [
        ...
        video_filter,
        "-af",
        f"atrim=end={duration:.9f},asetpts=PTS-STARTPTS",
        "-c:v",
        "libx264",
        "-pix_fmt",
        "yuv420p",
...
def main(
    *,
    default_steps: int = 8,
    default_lora_path: Path | None = DEFAULT_LORA_PATH,
    default_output_dir: Path = DEFAULT_OUTPUT_DIR,
    turbo_mode: bool = True,
) -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Render GT/render stage-1 classification pairs with MiniMax-H3 Turbo FL2AV"
            if turbo_mode
            else "Render GT/render stage-1 classification pairs with MiniMax-H3 FL2AV"
        )
    )
    ...
    print(
        f"[render] fps={args.fps}, requested_size={args.width}x{args.height}, "
        f"aligned_size={render_width}x{render_height}, steps={args.num_inference_steps}, "
