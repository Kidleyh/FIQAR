...
- **Minimal eight-rank compatibility changes:** only `rollout_zero3.py`, `rollout_zero3.sh`, and `online_zero3.py` changed. The rollout entrypoint now accepts an explicit `--rollout-world-size` (minimum 2), checks the actual Accelerate world size against it, records it in the condition/denoise/final artifact contracts, and removes the former `world_size == 2` restriction. The launcher accepts any `NUM_PROCESSES >= 2` and forwards it. The online merge validates `rollout_world_size` and `rollout_zero_stage`. Defaults remain two ranks, so Phase-10/11 behavior is preserved. `train_zero3.py`, DeepSpeed configs, NFT math, policy roles, EMA, optimizer/checkpoint logic, reward, single-GPU paths, and core H3 files were not modified.
- **Real configuration:** run `outputs/minimax_h3_diffusionnft_zero3/phase12_8gpu_k8_20260904_090244` used `GPU_IDS=0,1,2,3,4,5,6,7`, rollout/train world size 8, K=8, seeds `0 1 2 3 4 5 6 7`, 1088x736, 175 frames, three inference steps, LoRA rank 4, learning rate `1e-4`, policy beta 1, KL beta `1e-4`, advantage clip 5, timestep fraction 0.99, old decay type 1, and `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`. The group remained one isolated prompt and produced no mp4.
- **Eight-rank rollout result:** all 16 rollouts across two iterations completed. Every seed launch logged ranks 0 through 7 at ZeRO stage 3, and all eight ranks jointly entered and completed all three H3 DiT denoise forwards. Each artifact has clean video latent shape `[1,24,52,46,68]`, clean audio latent shape `[2,32,292]`, exact condition image, in-memory MagFace reward, complete Phase-8.1 state/provenance, and valid latent/condition SHA-256. Per-rank rollout peak allocation was `15,529.25 MiB` for the base iteration and `15,531.59 MiB` for the old-LoRA iteration; peak reservation was `17,190 MiB` and `17,852 MiB`, respectively. No seed showed increasing allocation across the serial sequence.
- **Iteration 0 — base policy, step `0 -> 1`:** rewards were `[21.865512916019984, 21.34633225486392, 21.12722737448556, 21.19799180257888, 21.620809668586368, 21.67575604575021, 21.899555115472705, 21.79238221758888]`. The merged rollout contained exactly eight unique seed records. Training assigned rank 0 through rank 7 to sample 0 through sample 7, processed two selected timesteps per rank, completed backward/optimizer/EMA, and committed checkpoint-1. Group positive/negative/policy/total losses were `0.86870193 / 0.86870193 / 4.34351015 / 4.34351015`; KL was zero at base initialization, gradient norm `0.08404083`, current delta `0.235204052094`, old decay `0.001`, and post-EMA current/old distance `0.000235198856`. Rollout/training/iteration wall times were `2715.75 / 311.58 / 3027.71 s`.
- **Iteration 1 — checkpoint-1 old policy, step `1 -> 2`:** all eight seed records used `checkpoint-1/old_lora.safetensors`, role `old`, global step before 1, and exact SHA-256 `0d75f6fea109de517492d39423ead61577211dc446c921f2f8890ff0a564051a`; the logs contain 64 successful `patched_modules=104` confirmations (8 ranks x 8 seeds). Rewards were `[21.734488260178342, 21.297940049852645, 21.735787300836474, 21.341241541362944, 21.502003102075488, 21.762320041656494, 21.95342443102882, 21.576957293919154]`. Resume restored the nonzero pre-step current/old distance `0.000235198856`; rank/sample assignment again covered exactly `0..7`. Group positive/negative/policy/total losses were `0.79506415 / 0.80029142 / 3.98640561 / 3.98640561`, KL `0.00186889`, current/reference prediction distance `0.04178105`, gradient norm `0.06999784`, current delta `0.201979548328`, old decay `0.002`, and post-EMA current/old distance `0.000404215894`. Rollout/training/iteration wall times were `2357.77 / 436.05 / 2794.25 s`.
- **Training VRAM:** iteration-0 per-rank peak allocation was `50,158.60 MiB`; maximum reservation across ranks was `53,074 MiB`. Iteration-1 per-rank peak allocation was `50,164.44 MiB`; maximum reservation was `52,956 MiB`. This is substantially below the Phase-11 two-GPU training peak of about `73.9 GiB` allocated per GPU while preserving the same large-video activation shape.
- **Checkpoint and integrity validation:** checkpoint-1 and checkpoint-2 each contain non-empty complete current/old LoRA files, committed `training_state.json`, eight BF16 ZeRO optimizer shards, and eight ZeRO model-state shards. Final online state is `completed_iteration=1`, `global_step=2`, and latest checkpoint-2. A strict post-run validation rehashed every latent and condition, verified all 16 complete seed states, checked K=8 geometry/scheduler/world-size/provenance contracts, matched every iteration-1 policy hash to checkpoint-1 old LoRA, and confirmed rank/sample coverage. Both logs contain zero traceback, CUDA OOM, DeepSpeed child failure, or NaN/Inf metric, and both optimizer steps report `optimizer_step=true` with frozen old/reference gradients.
- **Status:** `8xH100 large-video DiffusionNFT online post-training verified`. No 8-rank-specific change to `train_zero3.py` or the DeepSpeed configuration was required. Current deliberate limitations remain sequential all-eight-GPU rollout per seed, single-machine execution, rank0-only VAE/reward finalization, and no rollout GPU-group scheduler.

### 2026-09-03 — phase 11 ZeRO-3 online DiffusionNFT loop (complete)

- **Modification goal and scope:** add an independent orchestration layer that serially runs the verified Phase-10 two-rank rollout once per seed, minimally validates/merges one same-prompt group, and invokes the verified Phase-9 two-rank NFT step. No rollout, reward, NFT-loss, scheduler, EMA, optimizer, checkpoint, DiT, VAE, or single-GPU online implementation was copied or changed.
- **Files:** added `examples/minimax_h3/model_training/diffusionnft/online_zero3.py` and `examples/minimax_h3/model_training/diffusionnft/train_online_zero3.sh`. The only Phase-10 compatibility change in `rollout_zero3.py` removes the seed-0 rejection and replaces the three seed-0 artifact/metadata literals with `args.seed`; one invocation still generates exactly one seed with the unchanged two-rank denoise, reward, artifact, and provenance paths.
- **Online flow and configuration:** the launcher exposes independent `GPU_IDS`, `ROLLOUT_WORLD_SIZE`, `TRAIN_WORLD_SIZE`, `GROUP_SIZE`, and `SEEDS`; defaults are two H100s, rollout/train world size 2, K=2 seeds `0 1`, 1088x736, 175 frames, three inference steps, and three target iterations. It enforces unique seeds, K equal to the seed count, `K % TRAIN_WORLD_SIZE == 0`, and enough visible GPU IDs. Each iteration computes `dataset_position = start + iteration % limit`, runs the seeds serially through `rollout_zero3.sh`, validates same prompt/geometry/schedulers/policy/reward provenance and non-empty latent/state/condition files, atomically writes the merged `rollout.json`, and delegates exactly one update to `train_zero3.sh`.
- **Policy provenance:** iteration 0 records base policy with no LoRA/checkpoint. Later iterations require the latest checkpoint's `old_lora.safetensors`, compute its SHA-256, and require every seed record to match role/path/hash/source-checkpoint/global-step before merge. Real iteration-1 and iteration-2 records exactly matched checkpoint-1 old SHA-256 `270e964ee3c1a76f9656db01aace0634b363df4f160f94abf69e1fc59cac8f13` and checkpoint-2 old SHA-256 `94471d6a2ad7e61c32c370a7808408c56d3ff1b56a671558c05777c288e0f1c9`; both ranks reported `patched_modules=104` for every old-policy rollout.
- **Atomic state and recovery:** `online_zero3_state.json` atomically records stable configuration, completed iteration, current transaction, latest checkpoint, global step, per-seed reward/artifact/timing/memory data, merged rollout, and training result. A valid per-seed rollout is reused while an incomplete/invalid seed directory is atomically archived; a valid merged rollout bypasses generation/reward; and a complete checkpoint is treated as the optimizer-step commit and advances online state without repeating the update. Recovery A was exercised when seed0 was reused and only the initially invalid seed1 artifact was regenerated. Recovery B was exercised after the first training attempt failed: both completed rollouts and the condition cache were reused, with no repeat generation/reward, and training alone resumed. Checkpoint/global-step commits then progressed without overwrite or duplicate optimizer steps.
- **Allocator correction at target size:** the first 1088x736x175 training attempt reached the first `accelerator.backward()` and failed in ZeRO-3 gradient reduce-scatter with CUDA OOM before any optimizer step/checkpoint. Both GPUs returned to 0 MiB and state remained at step 0. Retrying the identical artifacts with `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` completed; this allocator-only setting is now the overridable launcher default. DeepSpeed configuration, parameter/optimizer placement, activation checkpointing, NFT math, and model code are unchanged.
- **One-iteration real result:** run `outputs/minimax_h3_diffusionnft_zero3/phase11_1iter_20260903_163331` used base-policy seed0/seed1 rewards `21.865512916019984` and `21.34633225486392`, merged K=2, assigned sample 0/1 to rank 0/1, completed two selected timesteps per rank, and advanced global step `0 -> 1`. Gradient norm was `0.16178128`, current delta `0.235894992701`, old decay `0.001`, current/old distance `0.000235888136`, and `optimizer_step=true`. Both training ranks peaked at `73,873.42 MiB` allocated and `76,666 MiB` reserved; checkpoint-1 contains complete current/old LoRA, training state, and DeepSpeed optimizer shards. The successful recovered training transaction took `445.31 s`; the earlier artifact-format corrections make the interrupted end-to-end attempt unsuitable as a clean rollout timing baseline.
- **Three-iteration real result:** the same run resumed checkpoint-1 and completed global steps `1 -> 2 -> 3`. Iteration 1 rewards were `[21.506229128156388, 21.15918963296073]`, rollout/training/total wall times were `1072.27 / 512.73 / 1585.28 s`, and checkpoint-2 completed with KL `0.00135233`, gradient norm `0.11533166`, current/old distance `0.000397908203`, and old decay `0.002`. Iteration 2 rewards were `[21.32374127705892, 21.262298107147217]`, wall times were `902.30 / 496.07 / 1398.51 s`, and checkpoint-3 completed with KL `0.00181205`, gradient norm `0.18959792`, current/old distance `0.000504335284`, and old decay `0.003`. Training allocation stayed flat at `73,896.88–73,896.89 MiB` per rank (`76,704 MiB` reserved); rollout allocation stayed flat at `39,228.82 MiB` per rank. No successful transaction contained OOM, NaN, Inf, or traceback, and both GPUs returned to 0 MiB after completion.
- **Artifact/result status:** all six seed records contain 1088x736x175 clean video/audio latents, exact condition images, in-memory rewards, Phase-8.1 reward provenance, and no mp4. All three merged groups isolate one prompt and two unique seeds. Checkpoints 1–3 each contain non-empty full `current_lora.safetensors`, `old_lora.safetensors`, `training_state.json`, and ZeRO-3 optimizer state. Final state is `completed_iteration=2`, `global_step=3`, and latest checkpoint-3.
- **Eight-GPU extension interface and limits:** orchestration does not equate rollout world size, training world size, group size, or visible GPU count. A future `GPU_IDS=0,...,7`, rollout world size 2, training world size 8, K=8 configuration therefore needs no state-contract rewrite. Phase 11 deliberately does not schedule multiple two-GPU rollout groups concurrently; seeds remain serial, reward remains rank0-only, and multi-node, sequence/context parallelism, CPU parameter/optimizer offload, and online rollout GPU-group scheduling remain out of scope.
